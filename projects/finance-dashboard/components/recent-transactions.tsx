...
export function RecentTransactions() {
  ...
  if (isLoading) {
    return (
      <div className="space-y-4 w-full" aria-live="polite" aria-busy={true}>
        {Array(4)
          .fill(0)
          .map((_, i) => (
  ...
  if (transactions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-6 text-center w-full" role="status">
  ...
  return (
    <div className="space-y-5 w-full">
      {transactions.map((transaction) => {
        ...
        return (
          <div key={transaction.id} className="flex items-center w-full">
            <div className="mr-4 rounded-full p-2.5 bg-muted flex-shrink-0" aria-hidden="true">
            ...
            <div
              className={`font-medium ${transaction.amount > 0 ? "text-green-500" : ""} flex-shrink-0 ml-2`}
